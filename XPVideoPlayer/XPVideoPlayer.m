...
NSString * const videoscheme = @"VideoStream";
...
@property (nonatomic, strong) XPVideoPlayerProvider *dataprovider;
...
- (void)setURL:(NSURL *)URL
{
    ...
    AVPlayerItem *playerItem = [[AVPlayerItem alloc] initWithAsset:self.urlasset];
...
- (void)setPlayerItem:(AVPlayerItem *)playerItem
{
    if (![playerItem.asset isKindOfClass:[AVURLAsset class]]) {
        NSLog(@"please init playerItem with an AVURLAsset object");
        return;
    }
...
#pragma mark - support cache

-(void)setSupportCache:(BOOL)SupportCache
{
    _SupportCache = SupportCache;
    if (SupportCache) {
        NSURL *url = [self GetCurrentURL];
        
        if(url == nil) return;
        [self resetPlayerItemIfNecessary];
        url = [self modifyURLScheme:url];
        
        NSLog(@"url:%@",url);
        
        self.dataprovider = [[XPVideoPlayerProvider alloc]initWithScheme:self.acturalscheme];
        self.urlasset = [AVURLAsset URLAssetWithURL:url options:nil];
        [self.urlasset.resourceLoader setDelegate:self.dataprovider queue:dispatch_get_global_queue(0, 0)];
    ...
    else
    {
        ...
        [self.urlasset.resourceLoader setDelegate:nil queue:nil];
    }
}
-(NSURL*)GetCurrentURL
{
    if(self.urlasset) return self.urlasset.URL;
    ...
    {
        NSLog(@"get current url failed");
        return nil;
    }
}
-(NSURL*)modifyURLScheme:(NSURL*)url
{
    NSLog(@"modifyurlscheme");
    self.acturalscheme = url.scheme;
    NSURLComponents *component = [[NSURLComponents alloc]initWithURL:url resolvingAgainstBaseURL:NO];
    [component setScheme:videoscheme];
